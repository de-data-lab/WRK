...
source(here("R/render_dumbbell.R"))
...
```{r, ref.label=c("housing-setup", "education-setup", "literacy-setup", "math-setup", "graduation-setup", "workforce-setup", "events-setup")}
...
graduation_gaps <- read_rds(here("data/processed/education_graduation_gaps.rds"))
...
# TODO: Combine this with the render_dumbell.R
graduation_gaps %>% 
  render_dumbbell_gap()
```
...
```{r}
# Plot Over time
event_counts_plot <- events_warehouse %>%
  ggplot(aes(x = month, fill = location)) +
  geom_histogram(binwidth = 0.5) + 
  scale_x_continuous(labels = ~month.abb[.], breaks = 1:12) + 
  facet_wrap(~year, ncol = 1) + 
  labs(x = NULL, y = NULL,
       title = "Number of Warehouse events over time",
       fill = "Top locations",
       caption = "Source: The Warehouse Public Calendar ")

# Render the plot
renderPlot({ event_counts_plot }, res = 100)
```
...
weekly_duration_plot <- events_weekly_duration %>%
  ggplot(aes(x = week, 
             y = weekly_sum, 
             group = 1)) +
  geom_line() + 
  ...
  labs(y = NULL,
       x = NULL,
       title = "Sum of weekly event hours over time")

# Render the plot
renderPlot({ weekly_duration_plot }, res = 90)
```


### Top events
```{r}
events_names_plot <- events_warehouse %>%
...
# Render plot
renderPlot({ events_names_plot }, res = 100)
```
