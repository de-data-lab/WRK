...
ELA_plot <- eastside_ELA %>%
  ...
  geom_hline(yintercept = 0,
             linetype = "dashed",
             color = tech_impact_colors("ti_blue")) + 
  annotate("text", x = 2016.5, y = 1.5, label = "Delaware Average", color = tech_impact_colors("ti_blue")) +
  scale_y_continuous(labels = ~paste0(., "%")) +
...
```{r}
valueBox(print_pct(eastside_math_gap_coeff / 100) %>% paste0("/year"), icon = "fas fa-calculator")
...
math_plot <- eastside_math %>%
  ...
  geom_point(size = 3, 
             color = tech_impact_colors("ti_orange")) +
  geom_hline(yintercept = 0, linetype = "dashed", color = tech_impact_colors("ti_blue")) + 
...
```{r}
valueBox(gap_yearly_change_2016 %>% paste0("/year"), icon = "fa-graduation-cap")
```
...
grad_gap_plot <- graduation_gaps %>%
  ...
  geom_smooth(data = subset(graduation_gaps, schoolyear >= 2016),
              method = "lm", se = FALSE,
              color = tech_impact_colors("ti_green"),
              linetype = "dotted") + 
  geom_segment(aes(x = schoolyear, xend = schoolyear, y = 0, yend = gap),
               size = 1.2) + 
  geom_point(size = 5) +
  ...
  annotate("text", x = 2015, y = 0.25,
           label = "Delaware Average", color = tech_impact_colors("ti_blue")) +
  labs(y = NULL,
       x = NULL,
