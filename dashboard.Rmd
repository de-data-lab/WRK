...
# 
# WRK_wide <- 
#   read_rds(here("data/processed/WRK_wide.rds"))
# 
# WRK_kinder <- WRK_wide %>%
...
### Improvement in literacy gap to Delaware
```{r}
# Load Data
...
```{r}
valueBox(print_pct(eastside_ELA_gap_coeff/100) %>% paste0("/year"), icon = "fa-book-open")
...
```{r}
valueBox(print_pct(eastside_math_gap_coeff / 100) %>% paste0("/year"), icon = "fa-pencil")
...
```{r}
valueBox(gap_yearly_change_2016 %>% paste0("/year"), icon = "fa-pencil")
```


Row 2: Proficiency  {.tabset .tabset-fade data-height=800}
-----------------------------------------------------------------------
### Summary
...
#   annotate("text", x = 2017, y = 0.65, label = "Delaware") + 
...
eastside_ELA %>%
  ...
  annotate("text", x = 2015, y = 1.5, label = "Delaware Average", color = "dodgerblue") +
  scale_y_continuous(labels = ~paste0(., "%")) +
  scale_x_continuous(expand = expansion(mult = 0.15), breaks = scales::breaks_pretty()) +
  labs(title = "Gaps with Delaware: East Side students' literacy achievement",
       x = NULL,
       y = NULL,
...
eastside_math %>%
  ...
  scale_y_continuous(labels = ~paste0(., "%")) +
  scale_x_continuous(expand = expansion(mult = 0.15), breaks = scales::breaks_pretty()) +
  labs(title = "Gaps with Delaware: East Side students' math achievement",
       x = NULL,
       y = NULL,
       caption = "Source: Delaware Open Data \nData points: East Side Charter students.") +
  theme_minimal()
...
renderPlot({grad_gap_plot}, res = 90)
```
